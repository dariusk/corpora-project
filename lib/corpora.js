...
var _categories = fs.readdirSync('node_modules/corpora/data')
  .map(function(dirname) {
    return {
      name: dirname,
      files: fs.readdirSync('node_modules/corpora/data/'+dirname).map(function(filename) {
        return {
          name: filename.replace('.json',''),
          get: function() {
            return JSON.parse(fs.readFileSync('node_modules/corpora/data/'+dirname+'/'+filename));
          }
        };
      })
    };
